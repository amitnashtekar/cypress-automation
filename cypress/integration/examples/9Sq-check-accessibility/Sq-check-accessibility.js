  ...
  describe('check for SQ-check component ', () => {
    ...
    it('user does sq-check for his address', () => {
      ...
      cy.findByRole('button', {
        name: /step 1 \- check what service is available at your address/i,
      }).click()
      cy.wait(5000)
    })

    it('user verifies the result post sq-check', () => {
      cy.hasElement('.labelcontainer label', () => {
        cy.get('.labelcontainer label')
          .eq(0)
          .then((address) => {
        ...
        cy.get('.content-fix.v-normal-top > .ux-button-sd > .primary')
          .click()
          .then(() => {
            cy.get('.sq-output').contains(this.seletedAddress.trim())
          })
      })
      cy.checkA11y(
        '.sq-output',
