...
describe("Information Rest Stub", function () {
    ...
    it('anaonymous information REST should have all desire atrrbutes in response', () => {
        cy.server()
        cy.route("GET", 'https://www.optus.com.au/api/cxf/au-loginservice/checkLogin*').as('authentication')
        ...
        // way 
       cy.get("@authentication").its("response.body.isLoggedIn").should("equal", false)
       cy.get("@authentication").its("response.body").should("deep.equal", {"isLoggedIn":false,"isServiceLevel":false,"isAccountLevel":false})
...
cy.route({
  ...
  delay: 100
       ...
       cy.get("@authentication").its("response.body.isLoggedIn").should("equal", true)
       cy.get("@authentication").its("response.body").should("deep.equal", {"isLoggedIn":true,"isServiceLevel":false,"isAccountLevel":true})
       cy.get("@authentication").should((xhr) => {
           console.log(xhr)
        expect(xhr.response.headers, 'response headers').to.include({
