...
describe("visit sq-check page", function () {
    ...
    describe("check for SQ-check component ", () => {
      ...
      it("user does sq-check for his address", () => {
       ...
       cy.findByRole('button', { name: /step 1 \- check what service is available at your address/i }).click()
      ...
      it("user verifies the result post sq-check", () => {
        ...
        debugger;
        cy.hasElement(".ux-radio-button .labelcontainer label", () => {
            cy.get(".ux-radio-button .labelcontainer label")
          .eq(0)
          .click()
          .then((selectedAddress) => {
            debugger;
            this.selectedAddress = selectedAddress[0].textContent.trim();
            ...
            cy.wait("@checkCoverage");
          });
          });
        
        
        // cypress will wait till this alias gets resolved
        ...
        //     .invoke("text")
        //     .then((el) => {
        //       expect(el).to.include(this.selectedAddress || '112 MAIN STREET, BLACKTOWN, NSW 2148');
        //     });
      });
      it("user verifies the result post sq-check in right way", () => {
          cy.get(".sq-output")
            .invoke("text")
            .then((el) => {
              expect(el).to.include(this.selectedAddress || '112 MAIN STREET, BLACKTOWN, NSW 2148');
